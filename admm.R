...
update_Z = function(st) {
  st$Z = st$rho / (2 * st$gamma + 3 * st$rho) * (0.5 * (st$Z1 + t(st$Z2)) + st$Z3 + st$Z4 - st$E2 - st$E3 - st$E4 + st$Psi2 + st$Psi3 + st$Psi4)
  st
}

update_Z1 = function(st) {
  n = nrow(st$Z1)
  Z1_tilde = (4 / 5) * (0.5 * st$Z + st$Z2 - 0.25 * t(st$Z2) + 0.5 * st$E4 + st$E1 - 0.5 * st$Psi4 - st$Psi1)
  Z1_new = matrix(0, n, n)
...
update_Z2 = function(st) {
  n = nrow(st$Z2)
  Z2_tilde = (4 / 5) * (st$Z1 - 0.25 * t(st$Z1) + 0.5 * t(st$Z) + 0.5 * t(st$E4) - st$E1 - 0.5 * t(st$Psi4) + st$Psi1)
  Z2_new = matrix(0, n, n)
...
update_Z3 = function(st) {
  n = nrow(st$Z3)
  st$Z3 = ((st$U + st$E8 - st$Psi8) %*% t(st$U1) + st$Z + st$E2 - st$Psi2) %*% solve(st$U1 %*% t(st$U1) + diag(n))
  st
}

update_Z4 = function(st) {
  L = nrow(st$edges)
  Z4_new = st$Z + st$E3 - st$Psi3
  ...
  for (l in seq_len(L)) {
    ...
    Z4_new[i, j] = (
      sum((st$A[l, ] + st$E9[l, ] - st$Psi9[l, ]) * d) +
        st$Z[i, j] + st$E3[i, j] - st$Psi3[i, j]
    ) / (sum(d * d) + 1)
  }
...
update_U = function(st) {
  st$U = (st$X + st$rho * (st$Z3 %*% st$U1 + st$U1 + st$U2 + st$U3 - st$E5 - st$E6 - st$E7 - st$E8 + st$Psi5 + st$Psi6 + st$Psi7 + st$Psi8)) / (1 + 4 * st$rho)
  st
}

update_U1 = function(st) {
  n = nrow(st$U1)
  st$U1 = solve(t(st$Z3) %*% st$Z3 + diag(n), t(st$Z3) %*% (st$U + st$E8 - st$Psi8) + st$U + st$E5 - st$Psi5)
  st
}
...
update_A = function(st) {
  ...
  for (l in seq_len(L)) {
    ...
    cvec = st$Z4[i, j] * (st$U2[i, ] - st$U3[j, ]) - st$E9[l, ] + st$Psi9[l, ]
    nc = sqrt(sum(cvec * cvec))

    if (nc > st$lambda / st$rho) {
      A_new[l, ] = (1 - st$lambda / (st$rho * nc)) *
        (st$rho / (st$lambda + st$rho)) * cvec
    }
  }
...
update_E1 = function(st) {
  st$E1 = st$rho / (st$mu + st$rho) * (st$Z1 - st$Z2 + st$Psi1)
  st
}

update_E2 = function(st) {
  st$E2 = st$rho / (st$mu + st$rho) * (st$Z3 - st$Z + st$Psi2)
  st
}

update_E3 = function(st) {
  st$E3 = st$rho / (st$mu + st$rho) * (st$Z4 - st$Z + st$Psi3)
  st
}

update_E4 = function(st) {
  st$E4 = st$rho / (st$mu + st$rho) * (0.5 * (st$Z1 + t(st$Z2)) - st$Z + st$Psi4)
  st
}
...
update_E6 = function(st) {
  st$E6 = st$rho / (st$mu + st$rho) * (st$U2 - st$U + st$Psi6)
  st
}

update_E7 = function(st) {
  st$E7 = st$rho / (st$mu + st$rho) * (st$U3 - st$U + st$Psi7)
  st
}

update_E8 = function(st) {
  st$E8 = st$rho / (st$mu + st$rho) * (st$Z3 %*% st$U1 - st$U + st$Psi8)
  st
}

update_E9 = function(st) {
  L = nrow(st$edges)
  E9_new = matrix(0, L, ncol(st$E9))

  for (l in seq_len(L)) {
    i = st$edges[l, 1]
    j = st$edges[l, 2]

    E9_new[l, ] = st$rho / (st$mu + st$rho) * (
      st$Z4[i, j] * (st$U2[i, ] - st$U3[j, ]) -
        st$A[l, ] +
        st$Psi9[l, ]
    )
  }
...
update_duals = function(st) {
  ...
  for (l in seq_len(L)) {
    i = st$edges[l, 1]
    j = st$edges[l, 2]
    Psi9_new[l, ] = Psi9_new[l, ] + st$Z4[i, j] * (st$U2[i, ] - st$U3[j, ]) - st$A[l, ] - st$E9[l, ]
  }

  st$Psi1 = st$Psi1 + st$Z1 - st$Z2 - st$E1
  st$Psi2 = st$Psi2 + st$Z3 - st$Z - st$E2
  st$Psi3 = st$Psi3 + st$Z4 - st$Z - st$E3
  st$Psi4 = st$Psi4 + 0.5 * (st$Z1 + t(st$Z2)) - st$Z - st$E4
  st$Psi5 = st$Psi5 + st$U1 - st$U - st$E5
  st$Psi6 = st$Psi6 + st$U2 - st$U - st$E6
  st$Psi7 = st$Psi7 + st$U3 - st$U - st$E7
  st$Psi8 = st$Psi8 + st$Z3 %*% st$U1 - st$U - st$E8
  st$Psi9 = Psi9_new
  st
}
...
stopping_criteria = function(st, st_prev) {
  ...
  R1 = st$Z1 - st$Z2 - st$E1
  R2 = st$Z3 - st$Z - st$E2
  R3 = st$Z4 - st$Z - st$E3
  R4 = 0.5 * (st$Z1 + t(st$Z2)) - st$Z - st$E4
  R5 = st$U1 - st$U - st$E5
  R6 = st$U2 - st$U - st$E6
  R7 = st$U3 - st$U - st$E7
  R8 = st$Z3 %*% st$U1 - st$U - st$E8
  ...
  for (l in seq_len(L)) {
    ...
    D9[l, ] = st$Z4[i, j] * (st$U2[i, ] - st$U3[j, ])
    R9[l, ] = D9[l, ] - st$A[l, ] - st$E9[l, ]
  }
